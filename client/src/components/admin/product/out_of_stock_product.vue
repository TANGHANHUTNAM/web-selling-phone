<template>
  <div class="mt-5">
    <div
      ...
      v-if="useAdminProduct.out_of_stock_products.length > 0"
    >
      <h2 class="text-center heading-product">DANH SÁCH SẢN PHẨM HẾT HÀNG</h2>
      <div class="row row-cols-lg-3 row-cols-md-3 row-cols-sm-2 row-cols-1">
        <div
          class="col g-3"
          v-for="product in useAdminProduct.out_of_stock_products"
          :key="product._id"
        >
          <div
    ...
    <div v-if="useAdminProduct.out_of_stock_products.length===0" class="text-center">
          ...
          <ul class="pagination d-flex">
            <li class="page-item">
              <a
                class="page-link"
                href="#"
                @click.prevent="useAdminProduct.backPage()"
                >Trở lại</a
              >
            </li>
            <li class="page-item">
              <a
                class="page-link"
                href="#"
                @click.prevent="useAdminProduct.nextPage()"
                >Tiếp theo</a
              >
            </li>
          </ul>
...
onMounted(() => {
  useAdminProduct.getOutOfStockProduct();
});
